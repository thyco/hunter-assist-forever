...
A hunter-only equipped ammunition indicator for WoW Forever (interface 16001). Version 0.4.1 retains the ammo indicator and Mongoose Bite / Counterattack glow. Range tinting has been removed.

## Install

Replace the existing `Interface/AddOns/HunterAssistForever` folder with the folder in `dist/HunterAssistForever-0.4.1.zip`, then reload the UI. Existing ammo settings are retained. Existing reactive glow settings are retained too; obsolete range settings are ignored.
